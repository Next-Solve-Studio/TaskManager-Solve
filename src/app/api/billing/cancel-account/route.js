...
export async function DELETE(request) {
    try {
        ...
        const response = await fetchWithTimeout(
            `${process.env.LICENSE_API_URL}/api/billing/cancel-account`,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { message: data.error || "Erro ao cancelar cadastro." },
        ...
        await Promise.all(
            userIds.map((uid) => auth.deleteUser(uid).catch(() => {})),
        );

        const lgpdResults = await Promise.allSettled(
            userIds.flatMap((uid) => [
                db.collection("google_tokens").doc(uid).delete(),
                db.collection("password_reset_codes").doc(uid).delete(),
                db.collection("password_change_codes").doc(uid).delete(),
        ...
        lgpdResults
            ...
            .forEach((r) => {console.error("[cancel-account] Falha LGPD ao limpar dado:", r.reason)})
        ...
        await Promise.all(
            projectsSnap.docs.map((d) => db.recursiveDelete(d.ref)),
        );
