...
export default function LoginMain() {
    ...
    const hideSidePanel = !haveAccount && registrationStep === 1;

    return (
        <main className="flex flex-row min-h-screen w-full overflow-hidden">
            <div
                className={`relative z-10 flex flex-col items-center justify-center
                min-h-screen bg-bg-main px-6 py-12 shrink-0
                transition-all duration-300
                ${hideSidePanel
                    ? "w-full"
                    : "w-full md:w-[45%] lg:w-[38%]"
                }`}
            ...
            >
                ...
                <div className={`form-panel-animate w-full ${hideSidePanel ? "max-w-3xl" : "max-w-105"}`}>
                    {haveAccount ? (
                        <LoginForm
