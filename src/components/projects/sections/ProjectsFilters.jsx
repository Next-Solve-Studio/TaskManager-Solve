...
export default function ProjectsFilters({
    ...
    return (
        <div className="flex flex-wrap gap-4 md:gap-2.5 py-3.5 px-4 bg-bg-card border border-white/10 rounded-[14px] items-center">
            ...
            {/* Select Dev */}
            <FormControl size="small" sx={{ minWidth: isMobile ? '100%' : 160, ...muiDark2 }}>
                <InputLabel>Dev</InputLabel>
                <Select value={filterDev} label="Dev" onChange={(e) => onDevChange(e.target.value)} MenuProps={menuPaper2}>
                <MenuItem value="all" style={{ color: "#e5e7eb" }}>Todos devs</MenuItem>
                {users.map((u) => (
                    <MenuItem key={u.id} value={u.id} style={{ color: "#e5e7eb" }}>{u.name}</MenuItem>
                ))}
                </Select>
            </FormControl>
...
function FilterSelect({ label, value, onChange, options, isMobile }) {
  return (
    <FormControl size="small" sx={{ minWidth: isMobile ? '100%' : 160, ...muiDark2 }}>
      ...
      <Select value={value} label={label} onChange={(e) => onChange(e.target.value)} MenuProps={menuPaper2}>
        <MenuItem value="all" style={{ color: "#e5e7eb" }}>Todos {label.toLowerCase()}</MenuItem>
        {Object.entries(options).map(([val, cfg]) => (
          <MenuItem key={val} value={val} style={{ color: cfg.color }}>{cfg.label}</MenuItem>
        ))}
      </Select>
    </FormControl>
  );
